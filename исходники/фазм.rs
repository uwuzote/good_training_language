...
fn сгенерировать_инструкции(файл: &mut impl Write, пп: &ПП, точка_входа_программы: usize) -> Результат<()> {
    ...
    for (индекс, инструкция) in пп.код.iter().enumerate() {
        ...
        match &инструкция.вид {
            ...
            ВидИнструкции::ВнешнийВызов{индекс, аргументы, результат} => {
                let регистры_цел = &["rdi", "rsi", "rdx", "rcx", "r8", "r9"];
                let mut номер_цел = 0;
                let регистры_вещ = &["xmm0", "xmm1", "xmm2", "xmm3", "xmm4", "xmm5", "xmm6", "xmm7"];
                let mut номер_вещ = 0;
                for аргумент in аргументы {
                    match аргумент {
                        Тип::Нат64 | Тип::Цел64 => {
                            if номер_цел >= регистры_цел.len() {
                                сделать!(&инструкция.лок, "Слишком большая арность");
                                return Err(());
                            }
                            let регистр = регистры_цел[номер_цел];
                            let _ = writeln!(файл, "    pop {регистр}");
                            номер_цел += 1;
                        }
                        Тип::Вещ32 => {
                            if номер_вещ >= регистры_вещ.len() {
                                сделать!(&инструкция.лок, "Слишком большая арность");
                                return Err(());
                            }
                            let регистр = регистры_вещ[номер_вещ];
                            let _ = writeln!(файл, "    pop r11");
                            let _ = writeln!(файл, "    movd {регистр}, r11d");
                            номер_вещ += 1;
                        }
                        _ => {
                            unreachable!("Передача аргумента типа «{}» во внешний вызов должна была быть поймана в компилятор.rs::вызвать_процедуру", аргумент.текст());
                            return Err(());
                        }
                    }
                }
...
pub fn сгенерировать_исполняемый_файл(путь_к_исполняемому: &Path, пп: &ПП, точка_входа_программы: usize, пути_библиотек: &[String]) -> Результат<()> {
    let статический = пп.внешние_символы.len() == 0;

    if статический && пути_библиотек.len() != 0 {
        eprintln!("ОШИБКА: даны пути библиотек для статической программы.");
        return Err(());
    }
