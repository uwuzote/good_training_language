...
pub enum ВидИнструкции {
    ...
    ВнешнийВызов{индекс: usize, аргументы: Vec<Тип>, результат: Option<Тип>},
    ...
    match &процедура.точка_входа {
        ...
        &ТочкаВхода::Внешняя{индекс} => {
            for параметр in &процедура.параметры {
                if параметр.тип != Тип::Нат64 && параметр.тип != Тип::Цел64 && параметр.тип != Тип::Вещ32 {
            ...
            код.push(Инструкция {
                вид: ВидИнструкции::ВнешнийВызов{
                    индекс,
                    аргументы: процедура.параметры.iter().map(|результат| результат.тип.clone()).collect(),
