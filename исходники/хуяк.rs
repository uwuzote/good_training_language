...
const КОМАНДЫ: &[Команда] = &[
    Команда {
        ...
        запустить: |программа, mut аргы| {
            ...
            let mut пути_библиотек = Vec::new();
            ...
            loop {
                match аргы.next() {
                    Some(арг) => match арг.as_str() {
                        ...
                        "-Б" => {
                            match аргы.next() {
                                Some(арг) => пути_библиотек.push(арг),
                                None => {
            ...
            if let Some(процедура) = программа.имена.процедуры.get(процедура_точки_входа) {
                ...
                фазм::сгенерировать_исполняемый_файл(&путь_к_исполняемому, &программа.пп, точка_входа, &пути_библиотек[..])?;
